...
std::vector<std::vector<float>> kmeans_clustering(
    ...
    for (int iter = 0; iter < max_iterations; ++iter) {
        ...
        for (size_t i = 0; i < data.size(); ++i) {
            ...
            for (int c = 0; c < k; ++c) {
                float dist = euclidean_distance_squared(data[i], centroids[c]);
                if (dist < min_dist) {
                    min_dist = dist;
                    closest_centroid = c;
                }
        ...
        float max_move = 0.0f;
        for (int c = 0; c < k; ++c) {
            float move = euclidean_distance_squared(centroids[c], new_centroids[c]);
            if (move > max_move) {
                max_move = move;
            }
        }
        ...
        if ((iter + 1) % 10 == 0) {
            std::cout << "K-means iteration " << iter + 1 << ", max move: " << max_move << std::endl;
        }
    }
...
std::vector<std::pair<int, int>> analyze_cluster_matches(
    ...
    const std::vector<std::vector<float>>& original_centroids,
    const std::vector<std::vector<float>>& reduced_centroids,
    const std::vector<std::vector<float>>& database_vectors,
    const std::vector<std::vector<float>>& reduced_vectors) {
    
    // 计算每个查询的groundtruth所属聚类中心（原始空间）
    ...
    for (size_t i = 0; i < queries.size(); ++i) {
        ...
        if (!groundtruth[i].empty()) {
            ...
            if (gt_id >= 0 && gt_id < database_vectors.size()) {
                ...
                for (size_t c = 0; c < original_centroids.size(); ++c) {
                    float dist = euclidean_distance_squared(database_vectors[gt_id], original_centroids[c]);
                    if (dist < min_dist) {
                        min_dist = dist;
                        closest_centroid = c;
                    }
    ...
    for (size_t i = 0; i < queries.size(); ++i) {
        ...
        for (int id : result_ids) {
            if (id >= 0 && id < reduced_vectors.size()) {
                float min_dist = std::numeric_limits<float>::max();
                int closest_centroid = -1;
                
                for (size_t c = 0; c < reduced_centroids.size(); ++c) {
                    float dist = euclidean_distance_squared(reduced_vectors[id], reduced_centroids[c]);
                    if (dist < min_dist) {
                        min_dist = dist;
                        closest_centroid = c;
                    }
...
std::pair<double, double> test_knn_search(
    ...
    const std::vector<std::vector<float>>& original_centroids,  // 原始空间聚类中心
    const std::vector<std::vector<float>>& reduced_centroids,  // 降维空间聚类中心
    const std::vector<std::vector<float>>& reduced_vectors,    // 降维后的向量
    int k,
    int nprob,
    int thread_count = std::thread::hardware_concurrency(),
    bool use_original_clusters = false,  // 是否使用原始空间聚类中心
    bool analyze_clusters = false) {  // 是否分析聚类
    ...
    // 存储每个查询在原始空间中最近的聚类中心
    std::vector<int> query_clusters(query_count, -1);
    
    // 如果使用原始空间聚类中心，先计算每个查询对应的聚类中心
    if (use_original_clusters && !original_centroids.empty()) {
        #pragma omp parallel for
        for (size_t i = 0; i < query_count; ++i) {
            ...
            for (size_t c = 0; c < original_centroids.size(); ++c) {
                float dist = euclidean_distance_squared(original_query_vectors[i], original_centroids[c]);
                if (dist < min_dist) {
                    min_dist = dist;
                    closest_centroid = c;
                }
            ...
            query_clusters[i] = closest_centroid;
        }
    }
    ...
    for (int t = 0; t < thread_count; ++t) {
        threads.emplace_back([&, t]() {
            ...
            for (size_t i = start_idx; i < end_idx; ++i) {
                ...
                std::string sql;
                if (use_original_clusters && query_clusters[i] != -1) {
                    // 基于原始空间聚类中心的查询策略
                    int target_cluster = query_clusters[i];
                    sql = "SELECT id, (vector <-> " + vec_str + ") as distance FROM sift_data "
                          "WHERE ivfflat_getlistindex(vector) = " + std::to_string(target_cluster) + " "
    ...
    std::cout << "Query strategy: " << (use_original_clusters ? "Original Space Clusters" : "Standard") 
              << ", nprob=" << nprob << ", recall@" << k << "=" << recall 
    ...
    if (analyze_clusters && !original_centroids.empty() && !reduced_centroids.empty()) {
        cluster_matches = analyze_cluster_matches(
            queries, groundtruth, all_topk_results, original_centroids, 
            reduced_centroids, original_vectors, reduced_vectors);
                    ...
                    if (id >= 0 && id < original_vectors.size()) {
                        ...
                        for (size_t c = 0; c < original_centroids.size(); ++c) {
                            float c_dist = euclidean_distance_squared(
                                original_vectors[id], original_centroids[c]);
                            if (c_dist < min_dist) {
                                min_dist = c_dist;
                                res_cluster = c;
                            }
...
int main(int argc, char* argv[]) {
    ...
    // 解析命令行参数
    int opt;
    while ((opt = getopt(argc, argv, "d:l:k:t:s:p:c:h")) != -1) {
        switch (opt) {
            case 'd':
            ...
            case 'c':
                use_original_clusters = (std::string(optarg) == "true");
                break;
            case 'h':
                std::cout << "Usage: " << argv[0] << " [options]\n"
                          ...
                          << "  -c <true/false> Use original space clusters for querying (default: false)\n"
    ...
    std::cout << std::endl;
    std::cout << "  Query strategy: " << (use_original_clusters ? "Original Space Clusters" : "Standard") << std::endl;
    ...
    // 在原始高维空间中计算聚类中心
    std::cout << "\nPerforming K-means clustering in original space..." << std::endl;
    auto original_centroids = kmeans_clustering(database_normalized, n_lists);
    std::cout << "Generated " << original_centroids.size() << " centroids in original space" << std::endl;
    ...
    // 获取降维空间中的聚类中心
    std::cout << "\nFetching IVFFlat centroids from reduced space..." << std::endl;
    auto reduced_centroids = get_ivfflat_centroids(conn, n_lists, target_dim);
    std::cout << "Successfully retrieved " << reduced_centroids.size() << " centroids of dimension " 
              << (reduced_centroids.empty() ? 0 : reduced_centroids[0].size()) << std::endl;
    ...
    std::cout << "Preprocessing time: " << pre_process_time << " seconds" << std::endl;
        ...
        // 执行实验
    std::cout << "\nPerforming benchmarking with both query strategies..." << std::endl;
    std::vector<BenchmarkResult> standard_results;
    std::vector<BenchmarkResult> original_cluster_results;
    
    // 首先测试标准查询策略
    std::cout << "\n=== Standard Query Strategy ===" << std::endl;
    for (int nprob : nprob_values) {
        std::cout << "\nTesting with nprob = " << nprob << std::endl;
        auto [recall, throughput] = test_knn_search(
            conn, queries_reduced, groundtruth, database, queries, 
            original_centroids, reduced_centroids, database_reduced,
            k, nprob, thread_count, false, true);
        
        standard_results.emplace_back(nprob, recall, throughput);
    }
    
    // 然后测试基于原始空间聚类中心的查询策略
    std::cout << "\n=== Original Space Cluster Query Strategy ===" << std::endl;
    for (int nprob : nprob_values) {
        std::cout << "\nTesting with nprob = " << nprob << std::endl;
        auto [recall, throughput] = test_knn_search(
            conn, queries_reduced, groundtruth, database, queries, 
            original_centroids, reduced_centroids, database_reduced,
            k, nprob, thread_count, true, true);
        
        original_cluster_results.emplace_back(nprob, recall, throughput);
    }
    
    // 输出汇总结果对比
    std::cout << "\n\n=== Summary Comparison ===" << std::endl;
    std::cout << "nprob\tStandard Recall@" << k << "\tOriginal Cluster Recall@" << k 
              << "\tStandard Throughput\tOriginal Cluster Throughput" << std::endl;
    
    for (size_t i = 0; i < standard_results.size(); ++i) {
        const auto& std_res = standard_results[i];
        const auto& oc_res = original_cluster_results[i];
        
        std::cout << std_res.nprob << "\t" << std_res.recall << "\t\t" 
                  << oc_res.recall << "\t\t" << std_res.throughput 
                  << "\t\t" << oc_res.throughput << std::endl;
    }
    
    // 计算平均提升
    double avg_recall_improvement = 0.0;
    double avg_throughput_impact = 0.0;
    int valid_comparisons = 0;
    
    for (size_t i = 0; i < standard_results.size(); ++i) {
        if (standard_results[i].nprob == original_cluster_results[i].nprob) {
            double recall_improv = (original_cluster_results[i].recall - standard_results[i].recall) / 
                                  standard_results[i].recall * 100.0;
            
            double throughput_impact = (original_cluster_results[i].throughput - standard_results[i].throughput) / 
                                     standard_results[i].throughput * 100.0;
            
            avg_recall_improvement += recall_improv;
            avg_throughput_impact += throughput_impact;
            valid_comparisons++;
            
            std::cout << "\nFor nprob=" << standard_results[i].nprob << ":" << std::endl;
            std::cout << "  Recall improvement: " << recall_improv << "%" << std::endl;
            std::cout << "  Throughput impact: " << throughput_impact << "%" << std::endl;
        }
    }
    
    if (valid_comparisons > 0) {
        avg_recall_improvement /= valid_comparisons;
        avg_throughput_impact /= valid_comparisons;
        
        std::cout << "\n=== Average Improvements ===" << std::endl;
        std::cout << "Average Recall Improvement: " << avg_recall_improvement << "%" << std::endl;
        std::cout << "Average Throughput Impact: " << avg_throughput_impact << "%" << std::endl;
    }
