...
def plot_results(flat_file, jl_file, sjlt_file, output_dir='plots'):
    ...
    plt.plot(nprobes, sjlt_throughput, 's-', label='Parallel SJLT')
    plt.xlabel('nprobe')
    ...
    plt.plot(nprobes, sjlt_recall, 's-', label='Parallel SJLT')
    plt.xlabel('nprobe')
    plt.ylabel('Recall')
...
def main():
    ...
    # 检查文件是否存在
    if not os.path.exists(args.flat_results):
        print(f"错误: 文件 {args.flat_results} 不存在")
        return
    
    if not os.path.exists(args.jl_results):
        print(f"错误: 文件 {args.jl_results} 不存在")
        return
