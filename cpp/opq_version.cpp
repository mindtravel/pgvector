...
// 计算协方差矩阵
Eigen::MatrixXf compute_covariance(const std::vector<std::vector<float>>& data) {
    int n = data.size();
    int dim = data[0].size();
...
// OPQ (Optimized Product Quantization) 类
class OPQuantizer {
private:
    ...
    Eigen::MatrixXf rotation_matrix;  // 旋转矩阵
    
public:
    OPQuantizer(int dim, int m, int k) 
        ...
        // 初始化旋转矩阵为单位矩阵
        rotation_matrix = Eigen::MatrixXf::Identity(original_dim, original_dim);
    }
    
    // 应用旋转矩阵
    std::vector<float> rotate(const std::vector<float>& vec) const {
        Eigen::VectorXf eigen_vec(original_dim);
        for (int i = 0; i < original_dim; i++) {
            eigen_vec(i) = vec[i];
        }
        
        Eigen::VectorXf rotated = rotation_matrix * eigen_vec;
    ...
    // 应用逆旋转
    std::vector<float> inverse_rotate(const std::vector<float>& vec) const {
        Eigen::VectorXf eigen_vec(original_dim);
        for (int i = 0; i < original_dim; i++) {
            eigen_vec(i) = vec[i];
        }
        
        Eigen::VectorXf rotated = rotation_matrix.transpose() * eigen_vec;
    ...
    void train(const std::vector<std::vector<float>>& data, int iterations = 10) {
        ...
        // 计算协方差矩阵并获取特征向量
        Eigen::MatrixXf cov = compute_covariance(data);
        Eigen::SelfAdjointEigenSolver<Eigen::MatrixXf> solver(cov);
        rotation_matrix = solver.eigenvectors().real();
        
        // 旋转数据
        std::vector<std::vector<float>> rotated_data(n_vectors);
        for (int i = 0; i < n_vectors; ++i) {
            rotated_data[i] = rotate(data[i]);
        }
        ...
        for (int m = 0; m < m_subvectors; ++m) {
            ...
            for (int i = 0; i < n_vectors; ++i) {
                ...
                for (int d = 0; d < subvector_dim; ++d) {
                    sub_vectors[i][d] = rotated_data[i][m * subvector_dim + d];
                }
            }
    ...
    std::vector<uint8_t> encode(const std::vector<float>& vector) const {
        ...
        // 先旋转向量
        std::vector<float> rotated_vec = rotate(vector);
        
        for (int m = 0; m < m_subvectors; ++m) {
            // 提取子向量
            std::vector<float> sub_vector(subvector_dim);
            for (int d = 0; d < subvector_dim; ++d) {
                sub_vector[d] = rotated_vec[m * subvector_dim + d];
            }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // 训练优化产品量化器
        std::cout << "\nTraining Optimized Product Quantizer..." << std::endl;
        OPQuantizer opq(128, m_subvectors, k_centroids);
        opq.train(database);
        std::cout << "Optimized Product Quantizer trained successfully" << std::endl;
        
        // 对数据进行OPQ编码
        std::cout << "Encoding dataset with OPQ..." << std::endl;
        auto database_pq_codes = opq.encode_all(database);
        auto queries_pq_codes = opq.encode_all(queries);
        std::cout << "OPQ encoding completed" << std::endl;
        ...
        // 输出JSON结果
        std::ofstream json_file("opq_results.json");
        if (json_file.is_open()) {
            ...
            json_file << "]" << std::endl;
            json_file.close();
            std::cout << "Results saved to opq_results.json" << std::endl;
        }
