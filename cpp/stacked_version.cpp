...
// 多层产品量化类
class StackedProductQuantizer {
private:
    int original_dim;     // 原始维度
    int m_subvectors;     // 每层子向量数量
    int k_centroids;      // 每个子空间的质心数量
    int n_layers;        // 层数
    int code_size;        // 编码大小（字节）
    
    std::vector<std::vector<std::vector<std::vector<float>>>> centroids;  // 质心表 [layer][m][k][d/m]
    
public:
    StackedProductQuantizer(int dim, int m, int k, int layers = 2) 
        : original_dim(dim), m_subvectors(m), k_centroids(k), n_layers(layers) {
        code_size = m_subvectors * n_layers;  // 每层子向量用一个字节表示
        
        // 初始化质心表
        int subvector_dim = original_dim / m_subvectors;
        centroids.resize(n_layers);
        for (int l = 0; l < n_layers; ++l) {
            centroids[l].resize(m_subvectors);
            for (int m = 0; m < m_subvectors; ++m) {
                centroids[l][m].resize(k_centroids, std::vector<float>(subvector_dim, 0.0f));
            }
        }
    }
    ...
    void train(const std::vector<std::vector<float>>& data, int iterations = 10) {
        ...
        // 逐层训练
        std::vector<std::vector<float>> current_data = data;
        
        for (int l = 0; l < n_layers; ++l) {
            std::cout << "Training layer " << l+1 << "/" << n_layers << std::endl;
            ...
            for (int m = 0; m < m_subvectors; ++m) {
                ...
                for (int i = 0; i < n_vectors; ++i) {
                    ...
                    for (int d = 0; d < subvector_dim; ++d) {
                        sub_vectors[i][d] = current_data[i][m * subvector_dim + d];
                    }
                }
                
                // K-means 聚类（简化版）
                std::vector<std::vector<float>>& current_centroids = centroids[l][m];
            ...
            // 如果不是最后一层，为下一层准备残差数据
            if (l < n_layers - 1) {
                std::cout << "Computing residuals for next layer..." << std::endl;
                std::vector<std::vector<float>> residuals(n_vectors, std::vector<float>(original_dim, 0.0f));
                ...
                for (int i = 0; i < n_vectors; ++i) {
                    ...
                    for (int m = 0; m < m_subvectors; ++m) {
                        ...
                        for (int d = 0; d < subvector_dim; ++d) {
                            sub_vector[d] = current_data[i][m * subvector_dim + d];
                        }
                        ...
                        for (int k = 0; k < k_centroids; ++k) {
                            float dist = euclidean_distance_squared(sub_vector, centroids[l][m][k]);
                            if (dist < min_dist) {
                                min_dist = dist;
                                closest_centroid = k;
                            }
                        ...
                        // 使用质心重建
                        for (int d = 0; d < subvector_dim; ++d) {
                            reconstructed[m * subvector_dim + d] = centroids[l][m][closest_centroid][d];
                        }
                    }
                    
                    // 计算残差
                    for (int d = 0; d < original_dim; ++d) {
                        residuals[i][d] = current_data[i][d] - reconstructed[d];
                    }
                }
    ...
    std::vector<uint8_t> encode(const std::vector<float>& vector) const {
        std::vector<uint8_t> codes(m_subvectors * n_layers);
        int subvector_dim = original_dim / m_subvectors;
        
        std::vector<float> current_vector = vector;
        ...
        for (int l = 0; l < n_layers; ++l) {
            for (int m = 0; m < m_subvectors; ++m) {
                ...
                std::vector<float> sub_vector(subvector_dim);
                for (int d = 0; d < subvector_dim; ++d) {
                    sub_vector[d] = current_vector[m * subvector_dim + d];
                }
                ...
                codes[l * m_subvectors + m] = static_cast<uint8_t>(closest_centroid);
            }
            ...
            if (l < n_layers - 1) {
                std::vector<float> reconstructed(original_dim, 0.0f);
                for (int m = 0; m < m_subvectors; ++m) {
                    int centroid_idx = codes[l * m_subvectors + m];
                    for (int d = 0; d < subvector_dim; ++d) {
                        reconstructed[m * subvector_dim + d] = centroids[l][m][centroid_idx][d];
                    }
                }
                
                // 计算残差
                for (int d = 0; d < original_dim; ++d) {
                    current_vector[d] = current_vector[d] - reconstructed[d];
                }
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "p:l:m:k:n:", long_options, NULL)) != -1) {
        switch (opt) {
            case 'p':
            ...
            default:
                std::cerr << "Usage: " << argv[0] 
                          ...
                          << "[-m m_subvectors] [-k k_centroids] [-n n_layers]" << std::endl;
                return 1;
        }
    }
    ...
    try {
        ...
        // 训练多层产品量化器
        std::cout << "\nTraining Stacked Product Quantizer with " << n_layers << " layers..." << std::endl;
        StackedProductQuantizer spq(128, m_subvectors, k_centroids, n_layers);
        spq.train(database);
        std::cout << "Stacked Product Quantizer trained successfully" << std::endl;
        
        // 对数据进行多层PQ编码
        std::cout << "Encoding dataset with Stacked PQ..." << std::endl;
        auto database_pq_codes = spq.encode_all(database);
        auto queries_pq_codes = spq.encode_all(queries);
        std::cout << "Stacked PQ encoding completed" << std::endl;
        ...
        // 输出JSON结果
        std::ofstream json_file("stacked_pq_results.json");
        if (json_file.is_open()) {
            ...
            json_file << "]" << std::endl;
            json_file.close();
            std::cout << "Results saved to stacked_pq_results.json" << std::endl;
        }
