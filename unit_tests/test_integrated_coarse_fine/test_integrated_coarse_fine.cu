...
// ClusterDataset 已移至 ../common/dataset/dataset.cuh
...
// ClusterDataset 的初始化和释放方法已移至 ../common/dataset/dataset.cuh
...
static std::vector<double> run_case(const BenchmarkCase& config,
    ...
    // 展平 query_batch，centroids 已经是连续存储的
...
int main(int argc, char** argv) {
    ...
    //     int k = 3;
        ...
        auto result = metrics.add_row_averaged([&]() -> std::vector<double> {
            auto metrics_result = run_case(config, &cached_dataset, cached_query_batch, nullptr);
