 ...
 * @param reordered_indices 重排后的索引数组 [n]，用于将重排位置映射回原始索引
...
inline void cpu_coarse_fine_search(
    ...
    const int* reordered_indices,  // [n] 重排后的索引数组，用于映射回原始索引
    ...
    auto worker = [&](int start, int end) {
        ...
        for (int qi = start; qi < end; ++qi) {
            ...
            // 输出结果（将重排后的位置索引映射回原始索引）
            for (int i = 0; i < topk && i < (int)fine_buffer.size(); ++i) {
                int reordered_pos = fine_buffer[i].idx;
                if (reordered_pos >= 0 && reordered_pos < n_total && reordered_indices != nullptr) {
                    out_index[qi][i] = reordered_indices[reordered_pos];
                } else {
                    out_index[qi][i] = -1;
                }
