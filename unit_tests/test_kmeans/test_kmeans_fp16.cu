...
static void cpu_kmeans_lloyd(
    ...
    const float* init_centroids,  // [k, dim]
    int* out_assign,              // [n]
    ...
    // init centroids from provided buffer
    std::memcpy(centroids.data(), init_centroids, sizeof(float) * k * d);
        ...
        for (int tid = 0; tid < num_threads; ++tid) {
            workers.emplace_back([&, tid]() {
                ...
                for (int i = start; i < end; ++i) {
                    ...
                    int best = 0;
                    float best_dist = l2_distance_squared(x, centroids.data(), d);
                    
                    // 找最近的 centroid
                    for (int c = 1; c < k; ++c) {
                        float dist = l2_distance_squared(x, centroids.data() + (size_t)c * d, d);
                        if (dist < best_dist) {
                            best_dist = dist;
                            best = c;
                        }
        ...
        for (int tid = 0; tid < num_threads; ++tid) {
            update_workers.emplace_back([&, tid]() {
                ...
                for (int c = start; c < end; ++c) {
                    float* cc = centroids.data() + (size_t)c * d;
                    if (counts[c] > 0) {
    ...
    std::memcpy(out_centroids, centroids.data(), sizeof(float) * k * d);
}
...
static std::vector<double> run_case(const KMeansCase& cfg) {
    ...
    MEASURE_MS_AND_SAVE("cpu_kmeans耗时:", cpu_ms,
        cpu_kmeans_lloyd(cfg,
                         h_data.data(),
                         h_init_centroids.data(),  // 使用相同的初始化聚类中心
                         h_assign_cpu.data(),
    ...
    float* d_centroids_out = nullptr;

    cudaMalloc(&d_assign, sizeof(int) * (size_t)n);
    cudaMalloc(&d_init_centroids, sizeof(float) * (size_t)k * dim);
    cudaMalloc(&d_centroids_out, sizeof(float) * (size_t)k * dim);
    // 将CPU初始化的聚类中心复制到GPU，确保CPU和GPU使用相同的初始化
    cudaMemcpy(d_init_centroids, h_init_centroids.data(), sizeof(float) * (size_t)k * dim, cudaMemcpyHostToDevice);

    float gpu_obj = 0.0f;
    double gpu_ms = 0.0;

    if (cfg.dtype == USE_FP32) {
        ...
        MEASURE_MS_AND_SAVE("gpu_kmeans耗时(fp32):", gpu_ms,
            gpu_kmeans_lloyd<float>(cfg, d_data, d_init_centroids, d_assign, d_centroids_out, &gpu_obj);
        ...
        half* d_data = nullptr;
        cudaMalloc(&d_data, sizeof(half) * (size_t)n * dim);

        // host -> fp16
        std::vector<half> h_data_h((size_t)n * dim);
        for (size_t i = 0; i < h_data.size(); ++i) h_data_h[i] = __float2half(h_data[i]);
        cudaMemcpy(d_data, h_data_h.data(), sizeof(half) * (size_t)n * dim, cudaMemcpyHostToDevice);

        MEASURE_MS_AND_SAVE("gpu_kmeans耗时(fp16):", gpu_ms,
            gpu_kmeans_lloyd<half>(cfg, d_data, d_init_centroids, d_assign, d_centroids_out, &gpu_obj);
            cudaDeviceSynchronize();
            CHECK_CUDA_ERRORS;
        );
    ...
    cudaMemcpy(h_centroids_gpu.data(), d_centroids_out, sizeof(float) * (size_t)k * dim, cudaMemcpyDeviceToHost);

    cudaFree(d_assign);
    cudaFree(d_init_centroids);
    cudaFree(d_centroids_out);
    CHECK_CUDA_ERRORS;
    ...
    {
        double s = 0.0;
        for (size_t i = 0; i < h_centroids_cpu.size(); ++i) {
            double d = (double)h_centroids_cpu[i] - (double)h_centroids_gpu[i];
            s += d * d;
        }
        centroid_rmse = std::sqrt(s / std::max<size_t>(1, h_centroids_cpu.size()));
    }

    double obj_tol = (cfg.dtype == USE_FP32) ? 1e-4 : 3e-3;
    double cen_tol = (cfg.dtype == USE_FP32) ? 1e-4 : 3e-3;

    bool pass = (centroid_rmse < cen_tol);
    // bool pass = (rel_obj_err < obj_tol) && (centroid_rmse < cen_tol);
...
int main(int argc, char** argv) {
    ...
    metrics.set_columns("pass_rate", "n", "k", "dim", "iters", "dist", "dtype",
    ...
    PARAM_3D(n, (20000, 1000000, 10000000),
             dim, (96),
            //  dim, (4,10,96,128,200),
            //  dtype, (0, 1))  // 0=fp32, 1=fp16
            //  dtype, (0))  // 0=fp32, 1=fp16
             dist, (COSINE_DISTANCE))
    {
        ...
        cfg.seed = 1234;
        cfg.dist = dist;
        cfg.dtype = (dtype == 0) ? USE_FP32 : USE_FP16;
        ...
        if (!QUIET) {
            ...
            COUT_VAL("KMeans Test: n=", cfg.n,
                        ...
                        " dtype=", (cfg.dtype == USE_FP32 ? "FP32" : "FP16"));
            COUT_ENDL("========================================");
        }
