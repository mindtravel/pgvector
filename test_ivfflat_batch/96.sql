...
-- ============================================
-- 第一部分：传统单个向量查询（20次查询，每次LIMIT 3）
-- ============================================
\echo ''
\echo '=== 第一部分：传统单个向量查询 ==='
\echo '执行3次单个向量查询，每次LIMIT 3...'

\timing on

\echo '查询1:'
SELECT id, category, description, embedding <-> :query1 as distance FROM test_vectors_perf ORDER BY embedding <-> :query1 LIMIT 3;

\echo '查询2:'
SELECT id, category, description, embedding <-> :query2 as distance FROM test_vectors_perf ORDER BY embedding <-> :query2 LIMIT 3;

\echo '查询3:'
SELECT id, category, description, embedding <-> :query3 as distance FROM test_vectors_perf ORDER BY embedding <-> :query3 LIMIT 3;

\echo '查询4:'
SELECT id, category, description, embedding <-> :query4 as distance FROM test_vectors_perf ORDER BY embedding <-> :query4 LIMIT 3;


-- ============================================
-- 第二部分：批量向量搜索（1次查询，包含24个向量，LIMIT 3）
-- ============================================
\echo ''
\echo '=== 第二部分：批量向量搜索 ==='
