 ...
 */
__device__ __forceinline__ float compute_l2_squared_float4_device(
 ...
 * 调用device函数进行计算
 */
__global__ void l2_squared_kernel_float4(
    const float* __restrict__ vectors, 
    float* __restrict__ vector_l2_squared, 
    int n_batch, 
    int n_dim) 
{
    ...
    /* 调用device函数计算平方和 */
    float sum = compute_l2_squared_float4_device(vec_ptr, n_dim, tid, blockDim.x);
...
) 
{
    ...
    // 计算L2范数
    if (tid == 0) {
        vector_l2_squared[bid] = shared_mem[0];
    }
}
    ...
    if (version != L2NORM_AUTO) {
        switch (version) {
            case L2NORM_BASIC: {
                ...
                if (stream) {
                    l2_squared_kernel_basic<<<grid_size, basic_block_size, 
                        basic_shared_mem, stream>>>(
                        const_cast<float*>(vectors), vector_l2_squared, n_batch, n_dim);
                } else {
                    l2_squared_kernel_basic<<<grid_size, basic_block_size, 
                        basic_shared_mem>>>(
                        const_cast<float*>(vectors), vector_l2_squared, n_batch, n_dim);
            ...
            case L2NORM_OPTIMIZED:
                if (stream) {
                    l2_squared_kernel<<<grid_size, block_size, 
                        shared_mem_size, stream>>>(
                        vectors, vector_l2_squared, n_batch, n_dim);
                } else {
                    l2_squared_kernel<<<grid_size, block_size, 
                        shared_mem_size>>>(
                        vectors, vector_l2_squared, n_batch, n_dim);
            ...
            case L2NORM_OPTIMIZED_V2:
                if (stream) {
                    l2_squared_kernel_optimized_v2<<<grid_size, block_size, 
                        shared_mem_size, stream>>>(
                        vectors, vector_l2_squared, n_batch, n_dim);
                } else {
                    l2_squared_kernel_optimized_v2<<<grid_size, block_size, 
            ...
            case L2NORM_OPTIMIZED_V3:
                if (stream) {
                    l2_squared_kernel_float4<<<grid_size, block_size, 
                        shared_mem_size, stream>>>(
                        vectors, vector_l2_squared, n_batch, n_dim);
                } else {
                    l2_squared_kernel_float4<<<grid_size, block_size, 
