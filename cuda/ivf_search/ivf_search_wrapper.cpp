...
extern "C" {
/**
 * C包装函数，用于从C代码调用ivf_search_pipeline（新接口：接受 device 指针）
 * 处理C++异常并返回错误码
 */
                                  ...
                                  int n_total_vectors, int n_probes, int k, int distance_mode)
{
    ...
    fprintf(stderr, "ivf_search_pipeline_wrapper: ========== 参数验证 ==========\n");
    ...
    /* 验证GPU内存指针 */
    fprintf(stderr, "ivf_search_pipeline_wrapper: GPU内存指针:\n");
    ...
    if (!d_query_batch || !d_cluster_size || !d_cluster_vectors || 
        ...
        fprintf(stderr, "ivf_search_pipeline_wrapper: 错误: GPU内存指针为NULL\n");
        return -1;
    }
    
    if (n_query <= 0 || n_dim <= 0 || n_total_cluster <= 0 || 
        n_total_vectors <= 0 || n_probes <= 0 || k <= 0) {
        fprintf(stderr, "ivf_search_pipeline_wrapper: 错误: 参数值无效\n");
        return -1;
    }
    
    if (n_probes > n_total_cluster) {
        fprintf(stderr, "ivf_search_pipeline_wrapper: 警告: n_probes(%d) > n_total_cluster(%d)\n",
                n_probes, n_total_cluster);
    }
    
    fprintf(stderr, "ivf_search_pipeline_wrapper: ========== 调用 ivf_search_pipeline ==========\n");
    ...
    try {
        ...
        fprintf(stderr, "ivf_search_pipeline_wrapper: 执行成功\n");
        return 0;
    } catch (const std::exception& e) {
        /* 记录异常信息到PostgreSQL日志 */
        fprintf(stderr, "ivf_search_pipeline_wrapper: 捕获到异常 - %s\n", e.what());
        return -1;
    } catch (...) {
        fprintf(stderr, "ivf_search_pipeline_wrapper: 捕获到未知异常\n");
        return -1;
    }
}
