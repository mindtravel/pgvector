...
#include "../fusion_l2_topk/fusion_l2_topk.cuh"
...
void batch_search_pipeline_choose(float* d_query_batch,
                           int* d_cluster_size,
    ...
    if (!d_cluster_size || !d_cluster_vectors || !d_cluster_centers || !d_query_batch) {
        fprintf(stderr, "[ERROR] batch_search_pipeline: 输入device指针为null\n");
        throw std::invalid_argument("input device pointers must not be null");
    }
    if (!d_topk_dist || !d_topk_index) {
        fprintf(stderr, "[ERROR] batch_search_pipeline: 输出device指针为null\n");
...

    {
        ...
        d_probe_vector_count = d_cluster_size;  // 直接使用传入的 device 指针
    ...
    
        {
            ...
            */
            if(mode == 0){
                ...
                pgvector::fusion_l2_topk_warpsort::fusion_l2_topk_warpsort<float, int>(
                    d_query_norm, d_cluster_centers_norm, d_inner_product, d_initial_indices,
                    n_query, n_total_clusters, n_probes,  // 粗筛选择 n_probes 个 cluster
                    d_top_nprobe_dist, d_top_nprobe_index,
                    true // select min 
        ...
        // ------------------------------------------------------------------
    {
        ...
        
        {
            ...
            if (mode == 0) {
                ...
                // 根据capacity选择kernel实例
                if (capacity <= 32) {
                    launch_indexed_inner_product_with_topk_kernel_v5_entry_based_l2<64, true, kQueriesPerBlock>(
                        block,
                        n_dim,
                        d_queries,
                        d_cluster_vectors_ptr,
                    ...
                    launch_indexed_inner_product_with_topk_kernel_v5_entry_based_l2<128, true, kQueriesPerBlock>(
                        block,
                        n_dim,
                        d_queries,
                        d_cluster_vectors_ptr,
                    ...
                    launch_indexed_inner_product_with_topk_kernel_v5_entry_based_l2<256, true, kQueriesPerBlock>(
                        block,
                        n_dim,
                        d_queries,
                        d_cluster_vectors_ptr,
