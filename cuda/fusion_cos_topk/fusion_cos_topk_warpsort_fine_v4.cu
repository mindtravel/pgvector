 ...
 * 
 * 根据数据规模动态选择最优算法：
 * - 大规模query：使用cublas_gemm + top-k选择
 * - 小规模query：使用v3流式实现
 */
void cuda_cos_topk_warpsort_fine_v4(
    ...
    
    {
        ...
        if (capacity <= 32) {
            launch_indexed_inner_product_with_topk_kernel_v4<64, true, kQueriesPerBlock>(
                block,
                n_dim,
                d_query_group,
                d_cluster_vector,
            ...
            launch_indexed_inner_product_with_topk_kernel_v4<128, true, kQueriesPerBlock>(
                block,
                n_dim,
                d_query_group,
                d_cluster_vector,
            ...
            launch_indexed_inner_product_with_topk_kernel_v4<256, true, kQueriesPerBlock>(
                block,
                n_dim,
                d_query_group,
                d_cluster_vector,
