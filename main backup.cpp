...
std::pair<double, double> test_knn_search(
    ...
    for (size_t i = 0; i < queries.size(); ++i) {
        ...
        std::string sql = 
            "SET ivfflat.probes = " + std::to_string(nprob) + ";\n"
            "SELECT id FROM sift_data ORDER BY vector <-> " + 
        ...
        total_recall += static_cast<double>(intersection.size()) / k;
        total_time += std::chrono::duration<double>(end - start).count();
        PQclear(res);
    }
    
    return {
        total_recall / queries.size(),
        queries.size() / total_time
    };
}
    ...
    try {
        ...
        for (int nprob : {1, 5}) {
            ...
            auto [recall, throughput] = test_knn_search(
                conn, queries, groundtruth, 10, nprob
            );
