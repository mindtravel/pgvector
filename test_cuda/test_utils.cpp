 ...
 * 
 */
bool matrix_equal_2D(float** a, float** b, int rows, int cols, float epsilon) {
...
float** generate_vector_list(int n_batch, int n_dim) {
    // 分配连续的内存
    float** vector_list = malloc_vector_list(n_batch, n_dim);
...
float*** generate_large_scale_vectors(int n_lists, int n_batch, int n_dim) {
    ...
    for (int list_id = 0; list_id < n_lists; list_id++) {
        // 为每个list分配连续内存
        vector_lists[list_id] = malloc_vector_list(n_batch, n_dim);
